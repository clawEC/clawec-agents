...
description: 通过 clawEC API 查询Shopee店铺榜单。在用户需要 Shopee店铺榜单、Shopee 相关数据查询时使用。
---
...
本技能调用 clawEC 开放 API，用于查询Shopee店铺榜单。
...
| date | body | 否 | 查询账期，yyyy-MM-dd；不传默认昨天；默认 `不传默认昨天` |
| shopType | body | 否 | 产品类型：1=虾皮优选 2=虾皮商城 3=其他 0=全部（默认0）；默认 `0` |
| borderType | body | 否 | 店铺类型：1=本土店铺 2=跨境店铺 0=全部（默认0）；默认 `0` |
| sortField | body | 是 | 排序字段：1=热销榜 2=飙升榜 |
| period | body | 是 | 周期：1=天 2=周 3=月 |
...
curl -s -X POST "https://www.clawec.com/api/aigc/ec/shopee/data/shop/ranking" \
  ...
  -d '{"site": "tw", "categoryId": "123456", "date": "不传默认昨天", "shopType": "0", "borderType": "0", "sortField": "1", "period": "1", "pageNo": "1", "pageSize": "10"}'
```
...
```bash
bash scripts/query.sh '{"site": "tw", "categoryId": "123456", "date": "不传默认昨天", "shopType": "0", "borderType": "0", "sortField": "1", "period": "1", "pageNo": "1", "pageSize": "10"}'
