...
BRANDNAME="${1:?用法: query.sh <brandName> [level1CategoryId] [brandSearchType]}"
LEVEL1CATEGORYID="${2:-}"
BRANDSEARCHTYPE="${3:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["brandName", "level1CategoryId", "brandSearchType"]'
_REQUIRED='["brandName"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${BRANDNAME}" "${LEVEL1CATEGORYID}" "${BRANDSEARCHTYPE}" <<'PY'
import json, os, sys
