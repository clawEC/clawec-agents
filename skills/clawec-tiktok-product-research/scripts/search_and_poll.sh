...
DAYS="7"
AI_INTERPRET="false"
POLL_INTERVAL="${TPR_POLL_INTERVAL:-4}"
MAX_ATTEMPTS="${TPR_POLL_MAX:-60}"
POSITIONAL=()
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
用法: search_and_poll.sh <category> [region] [days] [--ai]

提交 TikTok 选品机会分析，定位历史记录，拉取详情；若 --ai 则轮询 AI 解读直到完成或超时。

环境变量:
  TPR_POLL_INTERVAL  轮询间隔秒数，默认 4
  TPR_POLL_MAX       最大轮询次数，默认 60
EOF
      exit 0
      ;;
    --)
...
if [[ -z "$CATEGORY" ]]; then
  echo "用法: search_and_poll.sh <category> [region] [days] [--ai]" >&2
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

python3 - "$CATEGORY" "$REGION" "$DAYS" "$AI_INTERPRET" "$POLL_INTERVAL" "$MAX_ATTEMPTS" "$SCRIPT_DIR" <<'PY'
import json
import os
import subprocess
import sys
...
category, region, days_raw, ai_raw, poll_interval_raw, max_attempts_raw, script_dir = sys.argv[1:8]
...
days = int(days_raw) if str(days_raw).strip() else 7
...
def param_matches(param):
    ...
    if (param.get("region") or "US").strip() != target["region"]:
        return False
    if (param.get("category") or "").strip() != target["category"]:
        return False
    try:
        item_days = int(param.get("days") or 7)
    except (TypeError, ValueError):
        item_days = 7
    if item_days != target["days"]:
        return False
    if bool(param.get("aiInterpret")) != target["aiInterpret"]:
        return False
    return True

search_args = [search_sh, target["category"], target["region"], str(target["days"])]
if ai_interpret:
