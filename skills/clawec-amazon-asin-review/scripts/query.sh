...
MARKETPLACE="${1:?用法: query.sh <marketplace> <asin> [starList] [typeList] [page] [size]}"
ASIN="${2:?用法: query.sh <marketplace> <asin> [starList] [typeList] [page] [size]}"
STARLIST="${3:-}"
TYPELIST="${4:-}"
PAGE="${5:-}"
SIZE="${6:-}"
...
_KEYS='["marketplace", "asin", "starList", "typeList", "page", "size"]'
_REQUIRED='["marketplace", "asin"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${ASIN}" "${STARLIST}" "${TYPELIST}" "${PAGE}" "${SIZE}" <<'PY'
import json, os, sys
