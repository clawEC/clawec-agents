...
ITEMIDS="${2:?用法: query.sh <site> <itemIds> <type> [timest]}"
TYPE="${3:?用法: query.sh <site> <itemIds> <type> [timest]}"
TIMEST="${4:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["site", "itemIds", "type", "timest"]'
_REQUIRED='["site", "itemIds", "type"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${SITE}" "${ITEMIDS}" "${TYPE}" "${TIMEST}" <<'PY'
import json, os, sys
