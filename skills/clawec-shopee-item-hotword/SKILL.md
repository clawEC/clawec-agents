...
description: 通过 clawEC API 商品热搜词分析。在用户需要 Shopee商品热搜词、Shopee 相关数据查询时使用。
---
...
| itemIds | body | 是 | 商品ID，多个使用英文逗号分隔，最多10个；默认 `1234567890,1234567891` |
| type | body | 是 | 类型：1=引流词 2=同类目热词；默认 `1` |
| timest | body | 否 | 查询账期，yyyy-MM-dd，只能选某一天；不传默认昨天；默认 `2026-05-26` |
...
curl -s -X POST "https://www.clawec.com/api/aigc/ec/shopee/data/item/hotword" \
  ...
  -d '{"site": "tw", "itemIds": "1234567890,1234567891", "type": "1", "timest": "2026-05-26"}'
```

或使用脚本：

```bash
bash scripts/query.sh tw "1234567890,1234567891" 1 2026-05-26
```
