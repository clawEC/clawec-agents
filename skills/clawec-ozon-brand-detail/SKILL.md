---
name: clawec-ozon-brand-detail
description: 通过 clawEC API 批量查询 Ozon 品牌详情（最多10个）。在用户需要 Ozon 品牌详情、品牌调研时使用。
---
...
本技能调用 clawEC 开放 API，用于按品牌名称批量查询详情（最多 10 个）。
...
`POST /aigc/ec/ozon/data/brand/detail`
...
```bash
curl -s -X POST "https://www.clawec.com/api/aigc/ec/ozon/data/brand/detail" \
