---
name: clawec-tiktok-shop-base-info
description: 通过 clawEC API Tiktok店铺基础信息 查询店铺快照：累计 GMV/销量、类目/地区排名、店龄、评分、跨境/本地属性等。在用户需要 Tiktok店铺基础信息、TikTok 相关数据查询时使用。
---
...
本技能调用 clawEC 开放 API，用于Tiktok店铺基础信息 查询店铺快照：累计 GMV/销量、类目/地区排名、店龄、评分、跨境/本地属性等。
...
`POST /aigc/ec/tiktok/data/shop/base_info`
...
```bash
curl -s -X POST "https://www.clawec.com/api/aigc/ec/tiktok/data/shop/base_info" \
