...
REGION="US"
AI_INTERPRET="false"
POLL_INTERVAL="${TSA_POLL_INTERVAL:-4}"
MAX_ATTEMPTS="${TSA_POLL_MAX:-60}"
POSITIONAL=()
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
用法: search_and_poll.sh <shop> [region] [--ai]

提交 TikTok 店铺分析，定位历史记录，拉取详情；若 --ai 则轮询 AI 解读直到完成或超时。

环境变量:
  TSA_POLL_INTERVAL  轮询间隔秒数，默认 4
  TSA_POLL_MAX       最大轮询次数，默认 60
EOF
      exit 0
      ;;
    --)
...
if [[ -z "$SHOP" ]]; then
  echo "用法: search_and_poll.sh <shop> [region] [--ai]" >&2
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

python3 - "$SHOP" "$REGION" "$AI_INTERPRET" "$POLL_INTERVAL" "$MAX_ATTEMPTS" "$SCRIPT_DIR" <<'PY'
import json
import os
import subprocess
import sys
...
shop, region, ai_raw, poll_interval_raw, max_attempts_raw, script_dir = sys.argv[1:7]
...
def param_matches(param):
    if not isinstance(param, dict):
        return False
    if (param.get("shop") or "").strip() != target["shop"]:
        return False
...
search_args = [search_sh, target["shop"], target["region"]]
if ai_interpret:
