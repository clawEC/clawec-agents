...
MARKETPLACE="${1:?用法: query.sh <marketplace> [includeKeywords] [date]}"
INCLUDEKEYWORDS="${2:-}"
DATE="${3:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["marketplace", "includeKeywords", "date"]'
_REQUIRED='["marketplace"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${INCLUDEKEYWORDS}" "${DATE}" <<'PY'
import json, os, sys
