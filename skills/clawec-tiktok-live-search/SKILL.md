---
name: clawec-tiktok-live-search
description: 通过 clawEC API Tiktok直播搜索 按标题/主播/店铺搜索直播场次。在用户需要 Tiktok直播搜索、TikTok 相关数据查询时使用。
---
...
本技能调用 clawEC 开放 API，用于Tiktok直播搜索 按标题/主播/店铺搜索直播场次。
...
`POST /aigc/ec/tiktok/data/live/search`
...
```bash
curl -s -X POST "https://www.clawec.com/api/aigc/ec/tiktok/data/live/search" \
