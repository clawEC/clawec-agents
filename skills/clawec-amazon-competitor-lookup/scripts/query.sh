...
MARKETPLACE="${1:?用法: query.sh <marketplace> [month] [brand] [sellerName] [asins] [keyword]}"
MONTH="${2:-}"
BRAND="${3:-}"
SELLERNAME="${4:-}"
ASINS="${5:-}"
...
_KEYS='["marketplace", "month", "brand", "sellerName", "asins", "keyword"]'
_REQUIRED='["marketplace"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${MONTH}" "${BRAND}" "${SELLERNAME}" "${ASINS}" "${KEYWORD}" <<'PY'
import json, os, sys
