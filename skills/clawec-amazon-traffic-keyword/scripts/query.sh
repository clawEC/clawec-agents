...
MARKETPLACE="${1:?用法: query.sh <marketplace> <asin> [page] [size]}"
ASIN="${2:?用法: query.sh <marketplace> <asin> [page] [size]}"
PAGE="${3:-}"
SIZE="${4:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["marketplace", "asin", "page", "size"]'
_REQUIRED='["marketplace", "asin"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${ASIN}" "${PAGE}" "${SIZE}" <<'PY'
import json, os, sys
