...
description: 通过 clawEC API 查询Shopee热搜词榜单。在用户需要 Shopee热搜词榜单、Shopee 相关数据查询时使用。
---
...
本技能调用 clawEC 开放 API，用于查询Shopee热搜词榜单。
...
| sortField | body | 否 | 榜单排序：1=热销榜 2=飙升榜；默认 `1` |
| period | body | 否 | 榜单周期：1=天榜 3=月榜；默认 `3` |
| pageNo | body | 否 | 当前页码，从1开始；不传默认1；默认 `1` |
| pageSize | body | 否 | 每页条数，最大10；不传默认10；默认 `10` |
...
curl -s -X POST "https://www.clawec.com/api/aigc/ec/shopee/data/word/ranking" \
  ...
  -d '{"site": "tw", "categoryId": "123456", "borderType": "0", "sortField": "1", "period": "3", "pageNo": "1", "pageSize": "10"}'
```
...
```bash
bash scripts/query.sh '{"site": "tw", "categoryId": "123456", "borderType": "0", "sortField": "1", "period": "3", "pageNo": "1", "pageSize": "10"}'
