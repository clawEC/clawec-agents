...
KEYWORDIDS="${1:?用法: query.sh <keywordIds> [period]}"
PERIOD="${2:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["keywordIds", "period"]'
_REQUIRED='["keywordIds"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${KEYWORDIDS}" "${PERIOD}" <<'PY'
import json, os, sys
