...
MARKETPLACE="${1:?用法: query.sh <marketplace> <keyword> [historyDate] [page] [size]}"
KEYWORD="${2:?用法: query.sh <marketplace> <keyword> [historyDate] [page] [size]}"
HISTORYDATE="${3:-}"
PAGE="${4:-}"
SIZE="${5:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["marketplace", "keyword", "historyDate", "page", "size"]'
_REQUIRED='["marketplace", "keyword"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${KEYWORD}" "${HISTORYDATE}" "${PAGE}" "${SIZE}" <<'PY'
import json, os, sys
