...
MARKETPLACE="${1:?用法: query.sh <marketplace> <keyword>}"
KEYWORD="${2:?用法: query.sh <marketplace> <keyword>}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["marketplace", "keyword"]'
_REQUIRED='["marketplace", "keyword"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${KEYWORD}" <<'PY'
import json, os, sys
