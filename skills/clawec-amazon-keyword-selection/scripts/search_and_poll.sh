...
POLL_INTERVAL="${KR_POLL_INTERVAL:-4}"
MAX_ATTEMPTS="${KR_POLL_MAX:-60}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

usage() {
  cat <<'EOF'
search_and_poll.sh [region] [--month yyyyMM] [--departments CSV] [--keyword TEXT] [--ai]

环境变量:
  KR_POLL_INTERVAL  轮询间隔秒数，默认 4
  KR_POLL_MAX       最大轮询次数，默认 60
EOF
}
...
ai = "--ai" in args
cmd = [search_sh, region, "--month", month]
if depts: cmd += ["--departments", depts]
if keyword: cmd += ["--keyword", keyword]
if ai: cmd.append("--ai")
unwrap_data(run_json(cmd))
target = {"region": region, "month": month, "keyword": keyword.strip(), "aiInterpret": ai}
log_id = None
for _ in range(10):
    for item in (unwrap_data(run_json([logs_sh,"1","20"])).get("items") or []):
        p = item.get("param") or {}
        if (p.get("region") or "US") == region and str(p.get("month") or "") == month and (p.get("keyword") or "").strip() == keyword.strip():
...
print(json.dumps({"status":"ok","logId":log_id,"param":detail.get("param"),"items":detail.get("items",[]),"aiStatus":detail.get("aiStatus"),"aiAnalysis":detail.get("aiAnalysis")}, ensure_ascii=False, indent=2))
