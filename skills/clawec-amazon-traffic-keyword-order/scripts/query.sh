...
MARKETPLACE="${1:?用法: query.sh <marketplace> [asins] [reverseType] [date] [page] [size]}"
ASINS="${2:-}"
REVERSETYPE="${3:-}"
DATE="${4:-}"
PAGE="${5:-}"
...
_KEYS='["marketplace", "asins", "reverseType", "date", "page", "size"]'
_REQUIRED='["marketplace"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${ASINS}" "${REVERSETYPE}" "${DATE}" "${PAGE}" "${SIZE}" <<'PY'
import json, os, sys
