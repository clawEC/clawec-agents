...
MARKETPLACE="${1:?用法: query.sh <marketplace> <asin> [month]}"
ASIN="${2:?用法: query.sh <marketplace> <asin> [month]}"
MONTH="${3:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["marketplace", "asin", "month"]'
_REQUIRED='["marketplace", "asin"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${ASIN}" "${MONTH}" <<'PY'
import json, os, sys
