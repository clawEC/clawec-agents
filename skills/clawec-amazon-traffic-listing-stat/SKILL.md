---
name: clawec-amazon-traffic-listing-stat
description: 通过 clawEC API 查询亚马逊 ASIN 关联流量结构。在用户需要关联流量统计、免费/付费关联占比分析时使用。
---
...
本技能调用 clawEC 开放 API，用于查询 ASIN 关联流量结构（免费/付费流量及各关联类型占比）。
...
`POST /aigc/ec/amazon/data/traffic/listing_stat`
...
```bash
curl -s -X POST "https://www.clawec.com/api/aigc/ec/amazon/data/traffic/listing_stat" \
