...
description: 通过 clawEC API 批量获取Shopee店铺详情。在用户需要 Shopee店铺详情、Shopee 相关数据查询时使用。
---
...
本技能调用 clawEC 开放 API，用于批量获取Shopee店铺详情。
...
| shopIds | body | 是 | 店铺ID，多个用英文逗号分隔，最多10个 |
| timest | body | 否 | 查询账期，yyyy-MM-dd，只能选某一天；不传默认昨天 |

注意：`shopIds` 店铺ID，多个用英文逗号分隔，最多10个
