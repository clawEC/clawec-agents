...
MARKETPLACE="${1:?用法: query.sh <marketplace> [month] [keywords]}"
MONTH="${2:-}"
KEYWORDS="${3:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["marketplace", "month", "keywords"]'
_REQUIRED='["marketplace"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${MARKETPLACE}" "${MONTH}" "${KEYWORDS}" <<'PY'
import json, os, sys
