...
ITEMIDS="${2:?用法: query.sh <site> <itemIds> [timest]}"
TIMEST="${3:-}"
API_KEY="${CLAWEC_API_KEY:?请设置环境变量 CLAWEC_API_KEY}"

_KEYS='["site", "itemIds", "timest"]'
_REQUIRED='["site", "itemIds"]'
PAYLOAD=$(KEYS="$_KEYS" REQUIRED="$_REQUIRED" python3 - "${SITE}" "${ITEMIDS}" "${TIMEST}" <<'PY'
import json, os, sys
