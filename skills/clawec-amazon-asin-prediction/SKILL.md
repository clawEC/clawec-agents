---
name: clawec-amazon-asin-prediction
description: 通过 clawEC API 查询亚马逊 ASIN 日/月销量与销售额预测。在用户需要销量预测、销额预测时使用。
---
...
本技能调用 clawEC 开放 API，用于查询 ASIN 日/月销量与销售额预测数据。
...
`POST /aigc/ec/amazon/data/asin/prediction`
...
```bash
curl -s -X POST "https://www.clawec.com/api/aigc/ec/amazon/data/asin/prediction" \
